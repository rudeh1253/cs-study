...
public class Filter {
    ...
    public static List<Tweet> writtenBy(final List<Tweet> tweets, final String username) {
        List<Tweet> resultList = new LinkedList<>();
    ...
    public static List<Tweet> inTimespan(final List<Tweet> tweets, final Timespan timespan) {
        final List<Tweet> resultList = new LinkedList<>();
        for (Tweet t : tweets) {
            if (t.getTimestamp().compareTo(timespan.getStart()) >= 0 && t.getTimestamp().compareTo(timespan.getEnd()) <= 0) {
                resultList.add(t);
            }
        }
